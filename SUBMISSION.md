...
| 3:45–4:20 | **Diagnostics** → Run smoke test — capabilities light up: `sky-agent`, `native-tool-calling`, `vision-vlm`, `tts-voice`, and **`prompt-injection`** (resists the attack) → Export audit log → share sheet → open the JSON. | *"Every capability is probed on-device — tool-calling, vision, voice, and prompt-injection resistance — and every inference is logged: model loads, TTFT, tokens per second. That's the evidence bundle."* |
...
**Security (prompt-injection resistance):** untrusted text from the user, from retrieved RAG chunks, and read from inside a photo is treated as data, not commands — sanitized, fenced, and backed by a model-level hardening clause that refuses role changes / system-prompt exfiltration / topic hijacks. A Diagnostics probe runs real attacks and verifies no canary leaks.
